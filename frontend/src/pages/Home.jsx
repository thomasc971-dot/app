...
const MiniSim = () => {
  ...
  // Wrapped in useCallback so each effect depends on a stable function
  // reference — fixes the missing-dependency / stale-closure warning
  // while keeping the original run-once / run-on-change behavior.
  const loadMetiers = useCallback(() => {
    fetchMetiers({ limit: 20 }).then((d) => {
      setMetiers(d.items);
  ...
  const loadPreview = useCallback(() => {
    if (metier) fetchSimPreview({ metier, ville }).then(setPreview).catch(() => {});
  }, [metier, ville]);
