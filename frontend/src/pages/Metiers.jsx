...
function FilterBar({ secteurs, secteur, onSelect }) {
  return (
    <section className="mb-8">
      <div className="container-md">
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 -mx-2 px-2">
          <button data-testid="filter-tous" onClick={() => onSelect("tous")} className={`shrink-0 rounded-full px-5 py-2 font-body text-sm border transition-colors ${secteur === "tous" ? "bg-navy text-cream border-navy" : "bg-white text-navy/70 border-navy/10 hover:border-navy/30"}`}>
            Tous les secteurs
          </button>
          {secteurs.map(s => (
            <button key={s.id} data-testid={`filter-${s.id}`} onClick={() => onSelect(s.id)} className={`shrink-0 rounded-full px-5 py-2 font-body text-sm border transition-colors ${secteur === s.id ? "bg-navy text-cream border-navy" : "bg-white text-navy/70 border-navy/10 hover:border-navy/30"}`}>
              {s.label}
            </button>
          ))}
        </div>
...
function MetierCard({ m, index }) {
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.02, 0.4) }}>
...
function MetierGrid({ items, loading, total }) {
  return (
    <section>
      <div className="container-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((m, i) => <MetierCard key={m.slug} m={m} index={i} />)}
        </div>
      </div>
    </section>
  );
...
export default function Metiers() {
  ...
  const loadSecteurs = useCallback(() => {
    fetchSecteurs().then(setSecteurs);
  }, []);

  useEffect(() => { loadSecteurs(); }, [loadSecteurs]);
  ...
  const loadMetiers = useCallback(() => {
    ...
    if (q) params.q = q;
    return fetchMetiers(params).then(d => {
      setItems(d.items);
      setTotal(d.total);
      setLoading(false);
    });
  ...
  useEffect(() => {
    const t = setTimeout(loadMetiers, 250);
  ...
  return (
    <div data-testid="metiers-page" className="pb-24">
      ...
      <FilterBar secteurs={secteurs} secteur={secteur} onSelect={setSecteur} />
      <MetierGrid items={items} loading={loading} total={total} />
    </div>
  );
}
