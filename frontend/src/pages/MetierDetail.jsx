...
export default function MetierDetail() {
  ...
  useEffect(() => { fetchMetier(slug).then(setM).catch(() => setM(false)); }, [slug]);
  if (m === false) return <div className="container-md py-24"><p className="font-body text-navy/60">Métier introuvable.</p></div>;
  if (!m) return <div className="container-md py-24"><p className="font-body text-navy/60">Chargement...</p></div>;
          ...
          <div className="container-md">
            ...
            {m.scene.imprevu && (
              <div className="mt-12 max-w-4xl bg-brick/5 border border-brick/20 rounded-[1.5rem] p-8">
                ...
                <p className="font-body text-navy/80 leading-relaxed">{decode(m.scene.imprevu)}</p>
              </div>
            )}
          </div>
        </section>
            ...
            {m.evolution?.length > 0 && (
              <AccordionItem value="evol" className="bg-white rounded-[1.5rem] border border-navy/5 px-6">
                ...
                <AccordionContent className="pt-2">
                  ...
                  {m.remunerationTexte && <p className="font-body text-navy/70 mt-6 leading-relaxed" dangerouslySetInnerHTML={{ __html: m.remunerationTexte }} />}
                </AccordionContent>
              </AccordionItem>
            )}
            {m.contexteEvolution && (
              <AccordionItem value="context" className="bg-white rounded-[1.5rem] border border-navy/5 px-6">
                ...
                <AccordionContent><p className="font-body text-navy/70 leading-relaxed pt-2" dangerouslySetInnerHTML={{ __html: m.contexteEvolution }} /></AccordionContent>
              </AccordionItem>
            )}
            {m.variabilite && (
              <AccordionItem value="var" className="bg-white rounded-[1.5rem] border border-navy/5 px-6">
                ...
                <AccordionContent><p className="font-body text-navy/70 leading-relaxed pt-2" dangerouslySetInnerHTML={{ __html: m.variabilite }} /></AccordionContent>
              </AccordionItem>
            )}
          </Accordion>
        </div>
